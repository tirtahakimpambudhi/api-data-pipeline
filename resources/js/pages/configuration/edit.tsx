...
export default function ConfigurationEditPage({
  ...
  const initial = useRef<{
    service_environment_id: number | string | '';
    channel_id: number | string | '';
  }>({
    service_environment_id:
      configuration.service_environment?.id ?? configuration.service_environment_id ?? '',
    channel_id:
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(
      configurationRoute.update({ configuration: configuration.id }).url,
      { preserveScroll: true }
    );
  };
  ...
  const isDirty =
    String(data.service_environment_id ?? '') !==
      String(initial.current.service_environment_id ?? '') ||
    String(data.channel_id ?? '') !==
      String(initial.current.channel_id ?? '');
