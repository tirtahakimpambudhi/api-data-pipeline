...
import { BreadcrumbItem, Configuration, Filters, PaginatedResponse } from '@/types';
...
function isPaginated<T>(val: object): val is PaginatedResponse<T> {
    ...
    const v = val as any;
    return Array.isArray(v.data) && typeof v.total === 'number' && typeof v.current_page === 'number' && typeof v.per_page === 'number';
}
...
export default function ConfigurationIndexPage({ configurations, filters }: PageProps) {
    ...
    const [currentPage, setCurrentPage] = useState<number>(() =>
        isPaginated<Configuration>(configurations) ? configurations.current_page : (filters?.page ?? 1),
    );

    const [itemsPerPage, setItemsPerPage] = useState<number>(() =>
        isPaginated<Configuration>(configurations) ? configurations.per_page : (filters?.size ?? 10),
    );
    ...
    const totalItems = isPaginated<Configuration>(configurations) ? configurations.total : (configurations as Configuration[]).length;
    ...
    const numberItem = numberItemOnPage(
        isPaginated(configurations) ? configurations.current_page : currentPage,
        isPaginated(configurations) ? configurations.per_page : itemsPerPage,
    );
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem asChild>
                                <Link href={configurationRoute.show({ configuration: (item as any).id }).url}>View Details</Link>
                            </DropdownMenuItem>
                            <DropdownMenuItem asChild>
                        ...
                        <div className="text-sm text-muted-foreground">
                            {isPaginated(configurations) ? (
                                <span>
                                    ...
                                    <strong>{(configurations as PaginatedResponse<Configuration>).total}</strong> items
                                </span>
                            ) : (
                                <span>
